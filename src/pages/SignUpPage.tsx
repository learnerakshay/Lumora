...
export function SignUpPage() {
  ...
  const handleProviderSignUp = async (provider: 'google' | 'github' | 'email') => {
    ...
    clearError();
    try {
      await signUpWithProvider(provider, emailInput);
      navigate('/workspaces');
    } catch (err) {
      ...
      <div className="w-full max-w-md space-y-8 bg-[#121824] border border-slate-800/90 p-8 rounded-xl shadow-2xl shadow-sky-950/20">
        <div className="text-center space-y-2">
          ...
          <p className="text-xs text-slate-400">
            Sign up to provision your AI knowledge workspace platform.
          </p>
        </div>
        ...
        <div className="pt-4 border-t border-slate-800/80 text-center">
          <p className="text-xs text-slate-400">
            Already have a developer identity?{' '}
            <Link to="/sign-in" className="text-sky-400 hover:underline font-medium">
              ACCESS PORTAL
            </Link>
          </p>
        </div>
