...
3. **Payments** — **Payments Phases 1–2 (backend money path) and Phase 3A (frontend/payment-client foundation) are COMPLETE and FROZEN; Payments Phase 3B (Premium Pricing + Real Checkout) is the current next phase.** See "Payments (Razorpay Orders — one-time 30-day access)" below for the full architecture, exactly what is verified, and what remains. Do not reopen Phases 1–2 or 3A except for a blocking integration bug.
...
### Payments Phase 3B — Premium Pricing + Real Checkout — NEXT

Premium FREE/CORE/MAX pricing UI; `/pricing` page + landing pricing section; **features and limits derived from `PLAN_LIMITS`** (via Phase 3A's `pricing-presentation.ts`) so marketing copy cannot drift from enforced limits; coupon Apply/Remove UX wired to `POST /quote`; Razorpay Standard Checkout integration wired to Phase 3A's checkout state machine and script loader; explicit payment states (creating → gateway → verifying → success / failed / dismissed — `dismissed` is not a failure); a real Test Mode purchase completed from the actual frontend. `/billing`, upgrade/expiry UX, and placeholder replacement (`SettingsModal.tsx`, `UsagePage.tsx`, `UsageLimitNotice.tsx`'s `ACTION_LABELS` bug) follow in Phase 3C. **Include UPI checkout validation** in Phase 3B/3C once the real frontend exposes the required flow.
