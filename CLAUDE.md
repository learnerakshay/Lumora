...
3. **Payments** — **Payments Phases 1–2 (backend money path) are COMPLETE and FROZEN; Payments Phase 3 (payment UI) is the current next phase.** See "Payments (Razorpay Orders — one-time 30-day access)" below for the full architecture, exactly what is verified, and what remains. Do not reopen Phases 1–2 except for a blocking integration bug.
...
### Payments Phase 3 — NEXT (frontend / payment UX)

Premium FREE/CORE/MAX pricing UI; `/pricing` page + landing pricing section; **features and limits derived from `PLAN_LIMITS`** so marketing copy cannot drift from enforced limits; coupon UX; Razorpay Standard Checkout integration; explicit payment states (creating → gateway → verifying → success / failed / dismissed — `dismissed` is not a failure); `/billing` with current plan, access-until, days remaining, and payment history; Renew Plan; Upgrade to MAX; expiry warnings; upgrade CTAs where users hit limits; responsive/mobile polish. **Include UPI checkout validation here** if the real frontend exposes the required flow. Existing placeholders that must stop lying: `SettingsModal.tsx` ("billing controls will be shown here"), `UsagePage.tsx` plan grid (no upgrade affordance), and `UsageLimitNotice.tsx` (its `ACTION_LABELS` still omits `SKILL_INTELLIGENCE` / `LEARNING_PATH`, rendering "FREE undefined capacity reached").
